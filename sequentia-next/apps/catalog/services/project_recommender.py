...
WEIGHTS = {
    "role_alignment": 0.20,
    "goal_alignment": 0.20,     # domain match against primary target domain
    "readiness_margin": 0.20,   # comfortably-unlocked projects rank a little higher
    "current_path_relevance": 0.15,
    "difficulty_fit": 0.10,
    "portfolio_value": 0.10,
    "interest_alignment": 0.05,  # bounded on purpose — interest nudges, never overrides role/domain
}
...
def _role_alignment(project: ProjectMeta, target_role: str) -> float:
    """Distinct from domain match — Web Development contains Frontend,
    Backend, and Full Stack roles that should not recommend each other's
    projects just because they share a domain (spec's explicit example).
    No target_roles tagged on the project = neutral, not a penalty (older/
    less-tagged catalog rows shouldn't be structurally disadvantaged)."""
...
def _current_path_relevance(project: ProjectMeta, path) -> float:
    """Uses the `path` parameter — previously accepted but never actually
    read. A project whose demonstrated skills overlap with what the learner
    is currently/recently working on (not just their eventual full skill
    set) should rank higher than an equally-unlocked but topically
    unconnected project."""
...
def _interest_alignment(project: ProjectMeta, primary_domain: str | None, explicit_interests: set[str]) -> float:
    """Bounded secondary signal (WEIGHTS['interest_alignment'] is small on
    purpose). An interest in Machine Learning while the goal is Frontend
    Developer must never make an ML project outrank a frontend one — this
    factor can only nudge among projects that already passed the domain/
    role gate, it cannot pull in an out-of-scope project."""
    if project.domain == primary_domain:
        return 0.0  # already fully credited via goal_alignment
